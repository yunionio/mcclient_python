...
def do_instancebackup_list(client, args):
    """ List all instancebackups"""
    page_info = utils.get_paging_info(args)
    instancebackups = client.instancebackups.list(**page_info)
    utils.print_list(instancebackups, client.instancebackups.columns)