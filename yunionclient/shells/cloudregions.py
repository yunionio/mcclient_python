...
def do_cloud_region_list(client, args):
    """ List all apps"""
    page_info = utils.get_paging_info(args)
    regions = client.cloudregions.list(**page_info)
    utils.print_list(regions, client.cloudregions.columns)

@utils.arg('id', metavar='<CLOUDREGIOn_ID>', help='ID of cloudregion to use')
@utils.arg('--domain', metavar='<DOMAIN>', required=False, help='cloud region domain')
@utils.arg('--show-emulated', action='store_true', help='show emulated')
def do_cloud_region_capability(client, args):
    """ List all apps"""
    kwargs = {}
    if args.domain:
        kwargs['domain'] = args.domain
    if args.show_emulated:
        kwargs['show_emulated'] = True
    capa = client.cloudregions.get_specific(args.id, 'capability', **kwargs)
